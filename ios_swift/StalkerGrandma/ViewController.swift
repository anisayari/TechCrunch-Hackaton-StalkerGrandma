...
class ViewController: UIViewController, ARSCNViewDelegate {
    ...
    var classifierIds: [String] = ["model_ferdi"]
    ...
    func configureCloudantAndVisualRecognition() {
        self.visualRecognition = VisualRecognition.init(apiKey: "", version: self.VERSION)
    ...
    private func updateNode(classes: [ClassifiedImage], position: SCNVector3, frame: ARFrame) {
        ...
        classifiedImage.classifiers.forEach { classifierResult in
            ...
            if(Double(score) > highestScore){
                ...
                name_to_check = (personWithHighScore?.classes[0].className)!
                name = (personWithHighScore?.name)!
            }
        }
        ...
        guard let existentFace = results.first else {
            ...
            let node = SCNNode.init(classificationId: name_to_check, position: position)
